...
1. `using-superpowers` - 대화 시작 시 사용할 스킬을 찾고 적용하는 기본 규칙
2. `brainstorming` - 창의적 작업, 기능 설계, 동작 변경 전에 의도와 요구사항 정리
3. `using-git-worktrees` - 현재 작업공간과 격리된 기능 작업이 필요할 때
4. `writing-plans` - 여러 단계 작업을 코드 변경 전에 실행 가능한 계획으로 분해
5. `executing-plans` - 이미 작성된 구현 계획을 체크포인트 기반으로 실행
6. `subagent-driven-development` - 독립 작업이 여러 개 있는 구현 계획을 현재 세션에서 병렬적으로 진행
7. `dispatching-parallel-agents` - 공유 상태나 순차 의존성이 없는 2개 이상 작업을 병렬 처리
8. `test-driven-development` - 기능/버그 수정 구현 전에 실패 테스트부터 작성
9. `systematic-debugging` - 버그, 테스트 실패, 예상치 못한 동작의 원인을 추적
10. `requesting-code-review` - 작업 완료 후 요구사항 충족 여부를 리뷰 요청
11. `receiving-code-review` - 리뷰 피드백을 검토하고 구현 여부를 판단
12. `verification-before-completion` - 완료, 수정, 통과를 말하기 전에 검증 명령 실행
13. `finishing-a-development-branch` - 테스트 통과 후 merge, PR, 정리 방식 결정

## 분류별 색인

| 분류 | 스킬 | 용도 |
| --- | --- | --- |
| 기본 규칙 | `using-superpowers` | 스킬 발견, 우선순위, 플랫폼별 사용 규칙 |
| 설계 | `brainstorming` | 구현 전에 의도, 요구사항, 설계 선택지 정리 |
| 작업 격리 | `using-git-worktrees` | 기능 작업용 독립 worktree 생성과 기준선 검증 |
